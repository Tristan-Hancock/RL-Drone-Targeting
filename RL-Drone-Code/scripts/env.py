...
class DroneNavEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # Set step parameters.
        self.step_length = 5.0  # velocity magnitude (m/s)
    ...
    def _get_goal_from_object(self):
        ...
        try:
            ...
            pos = pose.position
            # Check if the pose is valid (AirSim may return NaN if not found).
            ...
            # Default goal in NED (example values – adjust based on your coordinates).
    ...
    def reset(self, seed=None, options=None):
        ...
        # Create start_pose explicitly using Python floats.
        ...
        # Takeoff and ascend sequence.
        print("[RESET] Taking off...")
        self.client.takeoffAsync(vehicle_name="Drone1").join()
        time.sleep(3)
        target_alt = float(self.start_pos[2])  # Convert to Python float (should be -10.0)
    ...
    def _get_obs(self):
        """Fetch the drone's current state and compute observation vector."""
    ...
    def _get_distance(self):
        """Compute Euclidean distance from drone to goal."""
        obs = self._get_obs()
    ...
    def step(self, action):
        ...
        """
        vx = vy = vz = 0.0
        if action == 0:   # Forward: +X direction in NED
            vx = self.step_length
            ...
            vy = -self.step_length
        elif action == 4: # Up: Ascend => in NED, ascend means more negative Z
            vz = -self.step_length
        elif action == 5: # Down: Descend (increase Z)
            vz = self.step_length
        elif action == 6: # Hover: all zero
            vx = vy = vz = 0.0

        # Command the drone to move with a given velocity for self.step_duration seconds.
        ...
        if self.prev_distance is not None:
            reward = (self.prev_distance - current_distance) * 10.0 - 1.0  # scaling plus time penalty
